...
def test_airtable_integration(mock_api):
    ...
    mock_table_instance.all.return_value = [
        {'fields': {'valuation_id': 1, 'valuation_date': '2022-01-01', 'expected_term': 3, 'public_comp_set': ['rec1'], 'share_price': 100, 'strike_price': 95, 'volatility_frequency': 'daily'}}
    ]
    
    # Mock the `table.get()` method to return a public company record for a given comp ID
    mock_table_instance.get.return_value = {
        'fields': {'ticker': 'AAPL'}
    }
    ...
    mock_api.return_value.table.side_effect = lambda base_id, table_name: mock_table_instance
    ...
    mock_table_instance.update.assert_called_once_with('rec123', {'option_value': 12.34})
...
def test_airtable_missing_fields(mock_api):
    mock_table_instance = MagicMock()
    mock_table_instance.all.return_value = [{'fields': {'valuation_id': 1, 'expected_term': 3, 'public_comp_set': ['rec1']}}]
    mock_api.return_value.table.side_effect = lambda base_id, table_name: mock_table_instance
    records = mock_table_instance.all()
    for record in records:
