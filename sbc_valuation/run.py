...
sorted_valuation_records = sorted(
    valuation_records, key=lambda x: x["fields"].get("valuation_id", 0)
)
...
for valuation_record in sorted_valuation_records:
    ...
    if not public_comps:
        print(
            f"Warning: No public_comp_set provided for valuation record {valuation['valuation_id']}. Skipping this record."
        )
        continue

    # Calculate the start and end dates for the volatility calculation
    end_date = datetime.strptime(valuation_date, "%Y-%m-%d")
    start_date = (end_date - timedelta(days=int(expected_term * 365))).strftime(
        "%Y-%m-%d"
    )
    ...
    for comp_id in public_comps:
        comp_record = public_comp_set_table.get(
            comp_id
        )  # Updated to use the public_comp_set_table instance
        ticker = comp_record["fields"]["ticker"]
        try:
            volatility = calculate_volatility(
                ticker, start_date, valuation_date, volatility_frequency.lower()
            )
            volatilities.append(
                {
                    "ticker": ticker,
                    "comp_id": comp_id,
                    f"{start_date} to {valuation_date}": volatility,
                }
            )
    ...
    if volatilities:
        average_volatility = round(
            np.mean([vol[f"{start_date} to {valuation_date}"] for vol in volatilities]),
            4,
        )
    else:
    ...
    if expected_term <= 1:
        ...
        risk_free_rate = (
            treasury_yields.get("1-year", np.nan)
            + (
                treasury_yields.get("5-year", np.nan)
                - treasury_yields.get("1-year", np.nan)
            )
            * (expected_term - 1)
            / 4
        ...
        risk_free_rate = (
            treasury_yields.get("5-year", np.nan)
            + (
                treasury_yields.get("10-year", np.nan)
                - treasury_yields.get("5-year", np.nan)
            )
            * (expected_term - 5)
            / 5
        ...
        risk_free_rate = (
            treasury_yields.get("10-year", np.nan)
            + (
                treasury_yields.get("30-year", np.nan)
                - treasury_yields.get("10-year", np.nan)
            )
            * (expected_term - 10)
            / 20
    ...
    print(
        f"Extrapolated risk-free rate for expected term {expected_term} years: {risk_free_rate}"
    )
    ...
    option_value = black_scholes(
        share_price, strike_price, expected_term, risk_free_rate, average_volatility
    )
    ...
    try:
        ...
        updated_valuation = option_valuations_table.update(
            valuation_record["id"], update_fields
        )
